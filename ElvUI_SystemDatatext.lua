...
local function OnEnter(self)
	if (E.db.sysdt.disableCombat and InCombatLockdown()) then
		return
	end
	...
	if IsShiftKeyDown() or not cpuProfiling then
		DT.tooltip:AddLine(" ")
		for i = 1, #memoryTable do
			if E.db.sysdt.maxAddons - shown <= 1 then
				break
			end
	...
	if cpuProfiling and not IsShiftKeyDown() then
		...
		for i = 1, #cpuTable do
			...
			if E.db.sysdt.maxAddons - shown <= 0 then
				break
			end
		end
		DT.tooltip:AddLine(" ")
...
local function InjectOptions()
	...
	E.Options.args.Crackpotx.args.impsysdt = {
		...
		get = function(info)
			return E.db.sysdt[info[#info]]
		end,
		set = function(info, value)
			E.db.sysdt[info[#info]] = value
			DT:LoadDataTexts()
		end,
		args = {
			maxAddons = {
